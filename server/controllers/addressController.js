    ...
    const [rows] = await pool.query(
        "SELECT * FROM dia_chi WHERE nguoi_dung_id=? ORDER BY mac_dinh DESC, ngay_cap_nhat DESC",
        [req.user.id]
    );
    ...
    try {
        ...
        if (mac_dinh) await conn.query("UPDATE dia_chi SET mac_dinh=0 WHERE nguoi_dung_id=?", [req.user.id]);

        const [r] = await conn.query(
        `INSERT INTO dia_chi
        (nguoi_dung_id, ho_ten, sdt, tinh_thanh, quan_huyen, phuong_xa, dia_chi, ghi_chu, mac_dinh)
        VALUES (?,?,?,?,?,?,?,?,?)`,
    ...
    try {
        await conn.beginTransaction();
        const [own] = await conn.query("SELECT id FROM dia_chi WHERE id=? AND nguoi_dung_id=? LIMIT 1", [id, req.user.id]);
    ...
    if (!req.user) return res.status(401).json({ ok:false, message:"Chưa đăng nhập" });
    const id = Number(req.params.id);
    const [own] = await pool.query("SELECT id FROM dia_chi WHERE id=? AND nguoi_dung_id=? LIMIT 1", [id, req.user.id]);
    if (!own.length) return res.status(404).json({ ok:false, message:"Không tìm thấy địa chỉ" });
    await pool.query("DELETE FROM dia_chi WHERE id=? AND nguoi_dung_id=?", [id, req.user.id]);
    ...
    try {
        ...
        await conn.query("UPDATE dia_chi SET mac_dinh=0 WHERE nguoi_dung_id=?", [req.user.id]);
        await conn.query("UPDATE dia_chi SET mac_dinh=1 WHERE id=? AND nguoi_dung_id=?", [id, req.user.id]);
